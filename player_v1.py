...
class Player(Entity):
    ...
    def step(self):
        ...
        # print('action', self.action, 'left_' in self.action[0], 'right_' in self.action[0], 'jump' in self.action[0])
        if 'left' in self.action[0]:
            self.ax -= self.move_speed
        if 'right' in self.action[0]:
            # print('right')
            self.ax += self.move_speed
        if 'jump' in self.action[0]:
            # print('right')
