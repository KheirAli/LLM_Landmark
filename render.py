...
from env import NudgeEnv

import matplotlib.pyplot as plt

# font_path = os.path.join(cv2.__path__[0], 'qt', 'fonts', 'DejaVuSans.ttf')
font = ImageFont.load_default(size = 40)
NudgeEnv = NudgeEnv(mode = 'getout', plusplus = False)
...
def render_getout(agent, args):
    ...
    def create_getout_instance(args, seed=None):
        ...
        getout = Getout()
        level_generator = ParameterizedLevelGenerator(enemies=enemies, flag = True)
    ...
    # print(type(getout.camera.screen))
    # frame rate limiting
    fps = 10
    ...
    while num_epi <= max_epi:
        ...
        if not getout.level.terminated:
            if args.alg == 'logic':
                ax.clear()
                ax.imshow(getout.camera.screen)
                if (getout.camera.screen) is not None:
                    getout.camera.screen.save('output_image.png', format='PNG')
                    image = imageio.imread("output_image.png")
                    image_list.append(image)
                    print(len(image_list))
                plt.pause(0.01)
                plt.show(block=False)
                t = agent.act(NudgeEnv.extract_logic_state(raw_state=getout))
                # print(t)
                action, explaining = t[0], t[1]
                # print(t[1])
                # quit()
                # action, explaining = agent.act(getout)
            elif args.alg == 'ppo':
                # print(getout)
                # getout.camera.screen.show()
                ax.clear()
                ax.imshow(getout.camera.screen)
                if (getout.camera.screen) is not None:
                    getout.camera.screen.save('output_image.png', format='PNG')
                ...
                plt.pause(0.01)
                plt.show(block=False)
                action = agent.act([NudgeEnv.extract_logic_state(raw_state=getout), NudgeEnv.extract_neural_state(raw_state=getout)])
            elif args.alg == 'human':
            ...
            print(len(image_list))
            imageio.mimsave(os.path.join('C:\\Users\\akheirandish3\\Nudge\\NUDGE', 'animation_{}.gif'.format(num_epi)), image_list, fps=5)
            image_list  = []
